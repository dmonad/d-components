...
const grab = num => {
  let str = ''
  for (let i = 0; i < num; i++, pos = (pos + 1) % lorems.length) {
    str = lorems[pos]
  }
  return str
}

export const defineLorem = component.createComponentDefiner(() => component.createComponent('d-lorem', {
  ...
  slots: state => ({
    content: `<div>${state.paragraphs.map(ps => `<p>${grab(ps)}</p>`).join('')}</div>`
  })
}))
